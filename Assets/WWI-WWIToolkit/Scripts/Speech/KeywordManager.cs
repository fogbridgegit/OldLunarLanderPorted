...
namespace WWIToolkit.Unity.Speech
{
    /// <summary>
        ...
        private class KeywordCommandInfo
        {
            ...
            public KeywordCommandInfo(string keyword)
            {
                if (string.IsNullOrEmpty (keyword)) {
                    UnityEngine.Debug.LogError("Attempted to register null keyword in keyword manager.");
                    return;
                }
                ...
                try {
                    ...
                    UnityEngine.Debug.Log("Unable to register keyword. Speech recognition may not be supported on this machine. Full exception: " + e.ToString());
                    Enabled = false;
                }
            }
            ...
            private KeywordConfidenceLevel WSAConfidenceToKeywordConfidence(ConfidenceLevel level)
            {
                ...
                switch (level)
                {
                    ...
                    default:
                        {
                            UnityEngine.Debug.LogError("Unknown Confidecne Level: " + level.ToString());
                            break;
                        }
                }
