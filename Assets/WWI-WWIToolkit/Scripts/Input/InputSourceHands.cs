...
namespace WWIToolkit.Unity.Input
{
    public class InputSourceHands : InputSourceBase, ITargetingInputSource
    {
        ...
        public void WSASourceUpdate(InteractionSourceUpdatedEventArgs state)
        {
            if (state.state.source.kind == InteractionSourceKind.Hand)
            {
                Vector3 newPosition;
                if (state.state.sourcePose.TryGetPosition(out newPosition))
                {
                    CurrentHandState inputState = trackedHands.Find(CurrentInputState => CurrentInputState.HandId == state.state.source.id); // handID
        ...
        public void WSASourceLost(InteractionSourceLostEventArgs state)
        {
            if (state.state.source.kind == InteractionSourceKind.Hand)
            {
                CurrentHandState inputState = trackedHands.Find(CurrentInputState => CurrentInputState.HandId == state.state.source.id); // handID
        ...
        private void WSAFingerPressed(InteractionSourcePressedEventArgs state)
        {
            CurrentHandState inputState = trackedHands.Find(CurrentInputState => CurrentInputState.HandId == state.state.source.id);
            if (inputState != null)
            {
